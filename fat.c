  ...
  if (-1 == get_dentry(disk_image, path, &dirEntry)) {
    ...
    while (rem_size > 0) {
      ...
      while (offset * content_length_per_line < CLUSTER_SIZE) {
        memcpy(content, &buf[offset * content_length_per_line],
               content_length_per_line);
...
int get_dentry_helper(int file_handle, struct msdos_dir_entry *result,
                      struct msdos_dir_entry *cur_dentry,
                      char *remaining_path) {
  if (cur_dentry == NULL) {
  ...
  if (rp_len < 1) {
    ...
    struct msdos_dir_entry *dep; // dir entry pointer
    if (-1 ==
        findUntilNext(search_entry_name, remaining_path, directory_seperator)) {
      remaining_path = "";
    }
    ...
    if (cur_dentry->attr !=
        type_volume) { // if it is the root it starts from cluster 2 but
    ...
    while (dep[i].name[0] != 0) {
      if (dep[i].name[0] == 229 /* 0xe5 */) {
        ...
        if (strcmp(dep_sname, search_entry_name) == 0) {
          ...
          // pln("\nRemainingPath:");
          // pln(remaining_path);
          return get_dentry_helper(file_handle, result, cur_dentry,
                                   remaining_path);
        }
...
int findUntilNext(char *result, char *str, char delimiter) {
  ...
  // printf("\n%s : %s : %d\n", result, str, i);
  if (str[i] == delimiter) {
    memcpy(str, &str[i + 1] /*don't include the delimiter*/,
           length < (i + 1) ? 0 : length - i - 1);
...
unsigned int readFAT(int file, u_int cnum) {
  ...
  u_int snum =
      fat_start_sector +
      (cnum / (SECTOR_SIZE /
               4 /*each one takes 4 bytes*/)); // since we expect nonnegative
